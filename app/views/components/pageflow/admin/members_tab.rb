    ...
    class MembersTab < ViewComponent
      def build(entry)
        embedded_index_table(entry.memberships.includes(:user).references(:users),
          ...
          table_for_collection class: 'memberships', sortable: true, i18n: Pageflow::Membership do |membership|
            column :user, sortable: "#{Pageflow.config.user_class.underscore.pluralize}.last_name", class: 'name'

  ...
  ActiveAdmin.register Entry, as: 'Entry' do
    ...
    controller do
      ...
      after_build do |entry|
        entry.account ||= current_pageflow_user.account #entry.account ||= account_policy_scope.entry_creatable.first || Account.first
      ...
      def permitted_attributes
        result = [:title]
        target = if !params[:id] && current_pageflow_user.admin?
                   Account.first
                 elsif params[:id]
                   resource
                 else
                   current_pageflow_user.accounts.first
                 end
